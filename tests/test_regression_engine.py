...
from krucible.domain.models import Attack, AttackResult, PolicyResult, Evaluation
from krucible.regression.comparator import PolicyStatusComparator, ToolUsageComparator, SemanticDriftComparator
...
def _create_evaluation(attack_id: str, response: str, status=PolicyResultStatus.PASS, tools=None) -> Evaluation:
    ...
    attack = Attack(id=attack_id, type="injection", name="Test", description="Test", payload="test", tags=[])
    ...
    return Evaluation(attack=attack, result=result, policy_results=[pol_res], passed=(status==PolicyResultStatus.PASS))
