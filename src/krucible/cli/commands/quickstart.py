    ...
    if journey_choice == 1:
        ...
        if provider_choice == 6:
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    m_choice = IntPrompt.ask(
                        "Select a model", choices=[str(i) for i in range(1, len(model_options) + 1)], show_choices=False
                    )
            ...
            try:
                if provider_choice == 1:
                    resp = requests.get(
                        "https://api.openai.com/v1/models", headers={"Authorization": f"Bearer {api_key}"}, timeout=5
                    )
                    if resp.status_code == 200:
                        available_models = [
                            m["id"]
                    ...
                    resp = requests.get(
                        f"https://generativelanguage.googleapis.com/v1beta/models?key={api_key}", timeout=5
                    )
                    if resp.status_code == 200:
                        available_models = [
                    ...
                    resp = requests.get(
                        "https://openrouter.ai/api/v1/models", headers={"Authorization": f"Bearer {api_key}"}, timeout=5
                    )
                    if resp.status_code == 200:
                        available_models = [m["id"] for m in resp.json().get("data", [])]
            except Exception:
            ...
            if not available_models:
                console.print(
                    f"[red]Failed to authenticate or fetch models for {adapter_name.title()}. Check your API key.[/red]"
                )
                raise typer.Exit(1)
            ...
            if Confirm.ask(f"Use {default_m}?"):
                ...
                m_choice = IntPrompt.ask(
                    "Select a model", choices=[str(i) for i in range(1, len(display_models) + 1)], show_choices=False
                )
        ...
        if found_key:
            ...
            if found_out_key:
                ...
                out_key = Prompt.ask(
                    "What is the JSON key containing the AI's response? (leave blank to use full body)", default=""
                )
                if out_key:
